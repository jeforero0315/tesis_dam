...
logging.basicConfig(filename='logs/access_control_ddl.txt', level=logging.INFO)
...
def has_privileges(user_name, query):
    """
    Verifica si el usuario tiene privilegios para ejecutar la consulta.
    """
    ...
    # Privilegios según el rol del usuario
    user_privileges = {
        'admin_tesis': ['FINANCIAL_DATA_USR.FINANCIAL_DATA'],  # admin tiene acceso completo
        'user_tesis': ['FINANCIAL_DATA_USR.FINANCIAL_DATA'],  # user solo tiene acceso a SELECT
    }
...
# Función para registrar la actividad de acceso
def log_access(user_name, query_text, status):
    ...
    with open('logs/access_control_ddl.txt', 'a') as file:
        file.write(log_message)
    logging.info(f"Acceso registrado: {log_message}")

# Función para recuperar registros de auditoría de consultas DDL
def query_audit_logs():
    """
    Recupera los registros de la tabla de auditoría de consultas DDL y ejecuta el control de acceso.
    """
    cursor = conn.cursor()
    try:
        # Recuperar registros de la tabla de auditoría
        cursor.execute("""
            SELECT timestamp, user_name, operation, object_name
            FROM SYS.audit_log
            ORDER BY timestamp DESC
        """)
        audit_logs = cursor.fetchall()
        logging.info(f"Se recuperaron {len(audit_logs)} registros de auditoría.")

        if not audit_logs:
            logging.info("No se encontraron registros en la auditoría.")
        ...
        for log in audit_logs:
            ...
            # Verificar privilegios para ejecutar la consulta
...
# Ejecutar el control de acceso y comenzar a escuchar las consultas
